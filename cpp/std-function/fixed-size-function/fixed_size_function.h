...
#ifndef FIXED_SIZE_FUNCTION_H_INCLUDED
#define FIXED_SIZE_FUNCTION_H_INCLUDED
...
template <typename Ret, typename... Args> struct fixed_function_vtable_base {
...
public:
  ...
  template <typename Functor> fixed_size_function(Functor &&f) {
    create(std::forward<Functor>(f));
  }

  template <typename Functor> fixed_size_function &operator=(Functor &&f) {
  ...
  template <typename Functor> void assign(Functor &&f) {
    reset();
    create(std::forward<Functor>(f));
  }
...
private:
  template <typename Functor> void create(Functor &&f) {
...
private:
  ...
  template <typename Functor> static void destroy_impl(void *functor) {
  ...
  template <typename Functor> static void move_impl(void *functor, void *dest) {
    new (dest) Functor(std::move(*static_cast<Functor *>(functor)));
  }

  template <typename Functor> void init_copy(std::true_type /*copyable*/) {
    vtable_.copy = &copy_impl<Functor>;
  }

  template <typename Functor> void init_copy(std::false_type /*copyable*/) {}

  template <typename Functor> void init_move(std::true_type /*movable*/) {
    vtable_.move = &move_impl<Functor>;
  }

  template <typename Functor> void init_move(std::false_type /*movable*/) {}
...
#endif // FIXED_SIZE_FUNCTION_H_INCLUDED
