...
def get_aqi_level(aqi):
    ...
    if aqi <= 50.0:
        str_aqi += '，优'
    elif aqi > 50.0 and aqi <= 100:
        str_aqi += '，良'
    elif aqi > 100.0 and aqi <= 150.0:
        str_aqi += '，轻度污染'
    elif aqi > 150.0 and aqi <= 200.0:
        str_aqi += '，中度污染'
    elif aqi > 200.0 and aqi <= 300.0:
        str_aqi += '，重度污染'
    else:
        str_aqi += '，严重污染'
    return str_aqi
...
def get_weather_info(city_code):
    ...
    if resp.status_code == 200 and is_json(resp) and resp.json().get('status') == 200:
        ...
        aqi = '今天空气质量(AQI): {}'.format(get_aqi_level(aqi))

        # yesterday air
        yesterday_aqi = yesterday_weather.get('aqi')
        yesterday_aqi = '昨天空气质量(AQI): {}'.format(get_aqi_level(yesterday_aqi))
...
if __name__ == '__main__':
    haidian = city_dict['海淀区']
    print(get_weather_info(haidian))
    chaoyang = city_dict['朝阳区']
    print(get_weather_info(chaoyang))
