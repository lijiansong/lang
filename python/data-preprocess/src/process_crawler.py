...
dicts_data = []
with open('../data/freeTop540Info.txt','r') as inf:
    for line in inf:
...
csv_file = currentPath + "/../data/freeTop540Info.csv"
WriteDictToCSV(csv_file, csv_columns, dicts_data)

# load CSV data into pd data frame
#df = pd.read_csv("../data/freeTop540Info.csv", names=['title', 'package_name', 'rating', 'downloads', 'category', 'short_desc', 'description'])
df = pd.read_csv("../data/freeTop540Info.csv")
print(df)
